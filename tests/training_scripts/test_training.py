...
from checkthat.task1.training_scripts.training import run_training, compute_metrics, load_config

@pytest.fixture
def mock_env():
    with patch("checkthat.task1.training_scripts.training.wandb.init") as mock_wandb_init, \
        patch("checkthat.task1.training_scripts.training.wandb.log") as mock_wandb_log, \
        patch("builtins.open", mock_open(read_data="parameters:\n  learning_rate:\n    values: [0.01]\n  batch_size:\n    values: [16]")), \
        patch("builtins.open", new_callable=MagicMock) as mock_file, \
        patch("yaml.safe_load", return_value={'parameters': {'learning_rate': {'values': [0.01]}, 'batch_size': {'values': [16]}}}) as mock_yaml_load, \
        patch("checkthat.task1.training_scripts.training.AutoModelForSequenceClassification") as mock_model, \
        patch("checkthat.task1.training_scripts.training.AutoTokenizer") as mock_tokenizer, \
        patch("checkthat.task1.training_scripts.training.Trainer") as mock_trainer:
        ...
        mock_trainer_instance.predict.return_value = MagicMock(metrics={"test_accuracy": 0.90})
...
def test_run_training_calls_external_functions(mock_env):
    ...
    model_path, tokenizer_path = run_training(
        train_dataset, eval_dataset, "bert-base-uncased", label_map, "en", test_dataset
    )
